					  ...
					  RPRNFAState *state, int64 matchEndRow)
{
	...
	/*
	 * Guards compare generations, not ctx->matchedState: FIN is not marked
	 * visited, so one DFS can record twice, and the free above hands the
	 * replaced address back to the allocator.
	 */
	ctx->matchGen++;
 ...
 * last) and each SEP.next is the next branch's content.  The branch link runs
 * from ALT through the SEP chain, so it never collides with a branch content
 * BEGIN's skip-past-END path.
 */
static void
