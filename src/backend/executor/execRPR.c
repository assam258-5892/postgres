 ...
 * read it at evaluation time, which now happens later during nfa_match(), so
 * it must NOT be restored here (unlike the former eager version).  The next
 * context's invalidation, or the next row's shared setup in
 * advance_reduced_frame_nfa, overwrites it.
 */
static void
