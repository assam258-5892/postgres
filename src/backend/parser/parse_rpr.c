					  ...
					  List **targetlist, List *groupClause)
{
	...
	foreach_node(ResTarget, restarget, windef->rpCommonSyntax->rpDefs)
	{
		 ...
		 * their own, which does not make them available that way, and
		 * reports them as ungrouped.  The stop reads groupClause rather than
		 * a sortgroupref, or the window's own ORDER BY would trip it in a
		 * query that does no grouping at all.
		 */
		ctx.pstate = pstate;
