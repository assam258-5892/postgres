...
replace_property_refs_mutator(Node *node, struct replace_property_refs_context *context)
{
	...
	else if (IsA(node, GraphLabelsRef))
	{
		 ...
		 * This allows the optimizer to push down predicates involving
		 * LABELS().
		 */
							...
							AttrNumber *labels_attnum, AttrNumber *property_names_attnum)
{
	...
	/*
	 * Set selectedCols to include only columns used by the element's property
	 * definitions for the matched labels.  This enables proper column-level
	 * privilege checking - the user only needs SELECT on columns that are
	 * actually referenced by the matched label's properties, not all columns
	 * of the table or all labels of the element.
	 */
