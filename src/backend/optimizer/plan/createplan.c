 ...
 *
 * XXX: TODO, the above comments need change.
 */
 ...
 * indices whose navigation reaches match_start (FIRST, LAST-with-offset, or
 * compound PREV_FIRST/NEXT_FIRST/PREV_LAST/NEXT_LAST-with-offset).  Such
 ...
 * resolved at executor init (eval_define_offsets), which can evaluate
 * non-constant offsets that the planner cannot fold.
 */
static void
