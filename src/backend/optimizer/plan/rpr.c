...
mergeConsecutiveAlts(List *children)
{
	...
	for (int outerpos = 0; outerpos < list_length(children); outerpos++)
	{
		...
		for (int restpos = outerpos + 1; restpos < list_length(children);)
		{
			...
			if (other->nodeType != RPR_PATTERN_ALT)
				break;
 ...
 * chain, so it never collides with a branch content BEGIN's skip-past-END
 * path, and each alternative's extent is bounded by its own trailing SEP
 * without consulting depth.
 *
 ...
 *
 * SEP markers serve the compile-time branch walk only; a branch's tail and a
 * branch-terminal group's BEGIN skip are redirected past the alternation.
 *
...
isUnboundedStart(RPRPattern *pattern, RPRElemIdx idx)
{
	 ...
	 * FIN bounds the walk: depth alone cannot when startDepth is 0.
	 */
