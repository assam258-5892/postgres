...
tryMultiplyQuantifiers(RPRPatternNode *pattern)
{
	...
	/*
	 * Flatten the child quantifier.  Both factors are bounds below
	 * RPR_QUANTITY_INF, so the widened product cannot overflow int64; the
	 * guard bounds the arithmetic rather than catching a reachable case, and
	 * declines the rewrite like the RPR_QUANTITY_INF gate beside it.
	 */
	if (pg_mul_s64_overflow(pattern->min, child->min, &new_min_64) ||
		new_min_64 >= RPR_QUANTITY_INF)
		return pattern;

	if (pattern->max == RPR_QUANTITY_INF || child->max == RPR_QUANTITY_INF)
		new_max_64 = RPR_QUANTITY_INF;
	else if (pg_mul_s64_overflow(pattern->max, child->max, &new_max_64) ||
			 new_max_64 >= RPR_QUANTITY_INF)
		return pattern;
