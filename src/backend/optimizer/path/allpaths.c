							   ...
							   Bitmapset *extra_used_attrs)
{
	...
	if (has_rpr)
	{
		 ...
		 * is a one-based index into windowClause and bounds
		 * find_window_functions(), so the list must keep its length and its
		 * order.  rpPattern stays too.  It holds no Vars, a pattern variable
		 * with no definition means TRUE, and leaving it is what keeps
		 * optimize_window_clauses() from mistaking an emptied RPR clause for
		 * a duplicate of a plain one.
		 *
