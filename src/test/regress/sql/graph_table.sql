...
-- Multi-pattern path tests (comma-separated path patterns with shared variables)
-- disconnected patterns: cross product with empty orders table
...
EXPLAIN (COSTS OFF) SELECT a_name, b_name, b_labels, c_name FROM GRAPH_TABLE (g1
    MATCH (a IS vl1)-[]->(b IS vl2|vl3),
          (b)-[]->(c IS vl3)
    WHERE 'vl2' = ANY(LABELS(b))
    COLUMNS (a.vname AS a_name, b.vname AS b_name, LABELS(b) AS b_labels, c.vname AS c_name)
);
