 ...
 * scan by resolve_nav_offsets().  The plan node pointer serves as lookup key;
 * ExecInitExprRec copies the values into the RPRNavState of each compiled
...
typedef struct RPRNavOffsets
{
	...
	RPRNavState *rprnavstate;	/* back-link, to pin resolved values per scan */
} RPRNavOffsets;
...
typedef struct RPRNFAContext
{
	...
	RPRNFAState *matchedState;	/* FIN state for greedy fallback, or NULL;
...
typedef struct WindowAggState
{
	 ...
	 * RPRNavExpr pointer; built by build_define_offsets()
	 */
	List	   *rprNavOffsets;
